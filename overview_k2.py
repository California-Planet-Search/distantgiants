...
def make_overview(plot = True, observability = False):
    ...
    ##################
    distantgiants = pd.read_csv('csv/k2_key_project.csv').rename(columns={'Name':'star_id'})
    distantgiants['ra'] = distantgiants['ra']*15
    ##################

    gamma_df = pd.read_csv('csv/K_Distant_Giants_gamma.csv')

    sql_df = pd.read_csv('csv/K_Distant_Giants_Observing_Requests.csv')[['star_id', 'instrument', 'counts', 'iodine_in', 'bjd']]
    ...
    # We want to find templates by excluding 10k recon spectra (on HIRES). Also take obs with counts < 0 b/c in May/Jun 2021 exposure meter was broken, so all obs have exp_count = -1 (and I didn't request and recon spectra). At most 9 systems have such templates (I can find them later by removing the counts < 0 option).
    ...
    # template_hires = sql_df.query('instrument == "hires_j" and (counts > 6e4 or counts < 0) and iodine_in == "f"').sort_values(by=['star_id', 'iodine_in', 'counts']).drop_duplicates(subset=['star_id', 'instrument'], keep = 'last')[['star_id', 'bjd', 'counts', 'iodine_in']].rename(columns = {'bjd':'hires_template_bjd'}).replace(np.nan, 0)
    ...
    # has_jitter just asks if a single star got 3 observations in a single (UT) day. Don't they need to be separated by ~1 hour? Maybe just on the same (UT) day is good enough.
    ...
    if plot == True:
        ...
        start_date = Time.now().jd - 2400
        end_date = Time.now().jd - 20
        
        sidereal_offset = 0.002731 # This is how much earlier a target rises each day (in days), ~3.9 minutes
        ...
        for i in range(len(plot_df)):
            ...
            if abs(plot_df['curv'][i]) > 0 and abs(plot_df['curv'][i]) >= 3*abs(plot_df['u_curv'][i]):
                curv_facecolors = 'k'
                print(plot_df['star_id'][i], plot_df['curv'][i], plot_df['u_curv'][i])
            else:
        ...
        ax.legend([recon_pts, jitter_pts, hires_template_pts, apf_template_pts, cadenced_hires_rvs, cadenced_apf_rvs], ['Recon', 'Jitter', 'HIRES Template', 'APF Template', 'HIRES', 'APF'], loc = (0.30,1.02), prop = {'size':10}, ncol = 4);
        fig.tight_layout()
