...
def make_overview(plot = False):
    ...
    # Add a new column 'cooked?' to indicate any target with a both a 3+ year baseline and 40+ observations. 
    cooked = []
    ...
    if plot == True:
        ...
        for i in range(len(plot_df)):
            ...
            if plot_df['bjd_recon'][i] < start_date:
                ax.scatter(start_date + 2, y_length[i] - 0.25*y_increment, marker = '<', color = recon_color, s=10, zorder = z_order_list.index('recon_arrows'))
    
            if plot_df['bjd_jitter'][i] < start_date:
                ax.scatter(start_date + 2, y_length[i] + 0.25*y_increment, marker = '<', color = jitter_color, s=10, zorder = z_order_list.index('recon_arrows'))
...
if __name__ == "__main__":
   ...
   update_overview(make_overview(plot = True))
